...
Dim objExcel, strExcelPath, objSheet, VERBOSE
Dim Update, FTPOnly, strLast, strFirst, strMiddle, strUserName, strPW, intRow, intCol
Dim strTel, strEmail, strStreet, strPOBox, strCity, strPostalCode, strCountry, strOrganisation, strDepartment
...
' Specify spreadsheet.
strExcelPath = "F:\Administration\Users\UsersProjects.xlsx"
...
' encountered in column 17 (the column for cn).
' For each row, create user and set attribute values.
strPreviousDN = ""

' Create log in windows explorer
...
objExplorer.document.write "Creating user accounts!<br>"
objExplorer.document.title = "Please be patient.... " 
objExplorer.document.write "<font color=black>"


strContainerDN = "ou=Researchers,dc=rre,dc=erasmusmc,dc=nl"

Do While objSheet.Cells(intRow, 6).Value <> ""


    ' Read values from spreadsheet for this user.
    If Trim(objSheet.Cells(intRow, 1).Value) = "1" Then
      Update = True
    Else
      Update = False
    End If

    If Trim(objSheet.Cells(intRow, 3).Value) = "" Then
    ...
    Else
        ...
        FTPOnly = 0
        DIM ProjectsArray
        ProjectsArray = Split(objSheet.Cells(intRow, 2).Value,",")
        
        DIM groupArray
        groupArray = Split(objSheet.Cells(intRow, 3).Value,",")
    
        strFirst = Trim(objSheet.Cells(intRow, 4).Value)
        strMiddle = Trim(objSheet.Cells(intRow, 5).Value)
        strLast = Trim(objSheet.Cells(intRow, 6).Value)
        strTel  = ""
        strEmail = Trim(objSheet.Cells(intRow, 9).Value)
        strOrganisation = ""
        strDepartment = ""
        strStreet = ""
        strCity = ""
        strPostalCode = ""
        strCountry = ""
        strUserName = Trim(objSheet.Cells(intRow, 7).Value)
        strPW = Trim(objSheet.Cells(intRow, 8).Value)
        ...
        ' Not used can be added if necessary as extra columns in Excell sheet
        strHomeFolder = ""
        strHomeDrive = ""
        strLogonScript = ""
            ...
            Else
                ...
                Else
                    ...
                    If (strOrganisation <> "") Then
                        objUser.company = strOrganisation
                    End If
...
'    		If ADS_UF_SMARTCARD_REQUIRED AND intUAC Then
'    		    objExplorer.document.write "Smartcard already required <br>" 
'    		Else
'     		   objUser.put "userAccountControl", intUAC OR ADS_UF_SMARTCARD_REQUIRED
'    	        End If
'    
'    		' Password does not expire
'    		If ADS_UF_DONT_EXPIRE_PASSWD AND intUAC Then
'    		    objExplorer.document.write "Password never expires already enabled <br>"
'    		Else
...
'    		End If
            	...
            	On Error Resume Next
                    ...
                    If (Err.Number <> 0) Then
                        On Error GoTo 0
                        objExplorer.document.write "<font color=red>Unable to set attributes for user with NT name: " _
                            & strNTName & "<br>"
                    End If
                    On Error GoTo 0
                    ' Create home folder.
                    If (strHomeFolder <> "") Then
                        ...
                        If (objFSO.FolderExists(strHomeFolder) = True) Then
                            ...
                            If (intRunError <> 0) Then
                                objExplorer.document.write "<font color=red>Error assigning permissions for user " _
                                    & strNTName & " to home folder " & strHomeFolder & "<br>"
                            End If
                        End If
                    End If
...
'objExplorer.document.write  "<font color=green><br> Don't forget to add a Token and FTP Access now! <br>"
'objExplorer.document.write  "<font color=red><br> Set smart card for interactive logon to yes for each new user! <br>"
